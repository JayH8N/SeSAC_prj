...
/*
 - 권한 요청
 - FaceID가 없다면?
    - 다른 인증 방법 권장 혹은 FaceID등록 권유. (아이폰 잠금을 아예 사용하지 않거나, 비밀번호만 등록한 사람)
    - FaceID를 설정하려면 아이폰 암호가 먼저 설정되어야 함. 그래서 아이폰 암호만 없는 경우는 없음
 - FaceID 변경? => domainStateData (안경, 마스크 등은 domainStateData가 변경X)
 - FaceID 계속 실패할 때? FallBack에 대한 처리가 필요. 다른 인증 방법으로 처리하기
 - FaceID 결과는 메인쓰레드 보장 X, DispatchQueue.main.async 필요
 - 한 화면에서, FaceID 인증을 성공하면, 해당 화면에 대해서는 success. (SwiftUI에서 state변경되면 body렌더링되서 뷰가 다시 그려지고 그럼 초기화. 다시 인증 필요)
 
 - 실제 서비스 테스트 + LSLP 생체인증 연동
 */
...
final class AuthenticationManager {
    ...
    func auth() {
        ...
        context.localizedCancelTitle = "FaceID인증 취소"
